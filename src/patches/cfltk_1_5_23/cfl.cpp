...
const char *Fl_dibo_analyze() {
    if (Fl::system_driver()==nullptr)
        return "system_driver is null";
    else {
        Fl_System_Driver* d = dynamic_cast<Fl_System_Driver*>(Fl::system_driver());
        if (d) return d->filesystems_label();
        return "bla";
    }
}
